...
function draw() {
  ...
  ctx.fillStyle = 'white';
  ctx.fillText(`${parseInt(mouseAngle / deg2Pi)}度`, 10, -10);
  ...
  ctx.save();
  ctx.rotate(mouseAngle - 10 * deg2Pi);
  ctx.lineTo(light_r, 0);
  ctx.rotate(20 * deg2Pi);
  ctx.lineTo(light_r, 0);
  ctx.fillStyle = '#ffcc60';
  ctx.fill();
  ctx.restore();

  let enemies = [{ r: 100, angle: 45 }, { r: 50, angle: -50 }, { r: 250, angle: 160 }, { r: 140, angle: -120 }];
  enemies.forEach(p => {
    ctx.save();
    ctx.beginPath();
    ctx.rotate(p.angle * deg2Pi);
    ctx.translate(p.r, 0);
    ctx.arc(0, 0, 5, 0, Math.PI * 2);

    //當角度介於燈光的角度跟距離區間時，把顏色變為紅色
    let color = Math.abs(p.angle * deg2Pi - mouseAngle) < 10 * deg2Pi && p.r < mouseDistance ? 'red' : 'white';
    ctx.fillStyle = color;
    ctx.fill();
    ctx.restore();
  });
