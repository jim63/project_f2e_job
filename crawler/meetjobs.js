    ...
    for (let page = 1; page <= totalPages; page++) {
      getJobsFromPage(page).then(data => {
        for (let i = 0; i < data.length; i++) {
          ...
          job_info.address = data[i].address.handwriting_street.replace(`'`, `''`);
          job_info.job_description = data[i].description.replace(`'`, `''`);
          ...
          //   console.log(`INSERT INTO job_meetjobs(job_name,salary,location,address,job_description,company_name,company_scale,link_job,link_company,appear_date,picture,job_id)
